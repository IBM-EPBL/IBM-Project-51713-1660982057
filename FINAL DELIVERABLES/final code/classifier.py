...
NUM_CLASSES = len(app.CLASSES)
IMAGE_SIZE = 100

def placeholder_inputs(batch_size):
    #Generates placeholders for the inputs
    images_placeholder = tf.compat.v1.placeholder(tf.float32, shape=(batch_size, IMAGE_SIZE, IMAGE_SIZE, 3), name="images")
    labels_placeholder = tf.compat.v1.placeholder(tf.int32, shape=(batch_size,), name="labels")
...
def training(loss, learning_rate, momentum = 0.75, beta1=0.9, beta2=0.999):
    ...
    optimizer = tf.train.optimizer.Adam(learning_rate, beta1=beta1, beta2=beta2)
...
def run_training(batch_size, learning_rate, epochs, run_number):
 ...
 #           summary_writer = tf.summary.FileWriter(logdir, session.graph)
            ...
            for step in range(epochs):
                training_data, training_labels = app.get_training_data()
                batches = [(training_data[i:i+batch_size], training_labels[i:i+batch_size]) for i in range(0, len(training_data), batch_size)]
                epochLoss = 0
                for batch in batches:
                    image_data = batch[0]
...
#                    summary_writer.flush()
                ...
                if (step + 1) % 5 == 0 or (step + 1) == epochs:
                    validation_data = app.get_validation_data(batch_size)
                ...
                if (step + 1) == epochs or early_stop:
                    ...
                    sys.stdout.flush()
                    test_data = app.get_test_data(batch_size)
